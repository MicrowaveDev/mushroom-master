...
export async function fetchFromTelegram() {
  const result = await runIncrementalFetch();
...
export async function regenerateFromTelegram(argv) {
  ...
  if (args.has('--render-existing-markdown')) {
    const { renderExistingLoreMarkdown } = await import('../../lib/render-existing-lore.js');
    const result = await renderExistingLoreMarkdown({ args: argv, sendPdf: !skipBot });
    process.stdout.write([
      ...
      result.botResults.length ? `Bot delivery targets: ${result.botResults.map((item) => item.chatId).join(', ')}` : 'Bot delivery skipped.'
    ].join('\n') + '\n');
    return;
  }
  ...
  const result = await runFullRegeneration(undefined, {
    ...
    skipDownload,
    sendPdf: !skipBot,
    template: readArgValue(argv, '--template')
  });
  process.stdout.write([
