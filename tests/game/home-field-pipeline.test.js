...
test('[home-field] chibi proof blocks failed local source before candidate commands', () => {
  ...
  assert.match(result.stdout, /one new or repair-approved supplied complete 8x4 state sheet after sourceGate is cleared/);
...
test('[home-field] imagegen requirements keep fallback methods out of default queue output', () => {
  ...
  assert.match(requirements, /run the queue script and use the printed queue results/);
...
test('[home-field] repo agent instructions distinguish sourceGate blockers from production readiness', () => {
  ...
  assert.match(agents, /use the printed queue results to generate what is needed/);
...
test('[home-field] chibi proof context prints narrow paths and commands', () => {
  ...
  assert.match(result.stdout, /until the sourcePath is replaced or a documented repair method is added/);
...
test('[home-field] structured generation queue encodes Thalla chibi gates', () => {
  ...
  assert.match(item.sourceGateRecovery.summary, /queue output owns the method-change instructions/);
  assert.match(item.sourceGateRecovery.copyablePrompt, /generation-queue -- --id=thalla-stage1-chibi-proof/);
  assert.match(item.sourceGateRecovery.copyablePrompt, /use the printed queue results to generate what is needed/);
  ...
  assert.ok(item.agentInstructions.some((instruction) => /new complete 8x4 local state-sheet source/.test(instruction)));
...
test('[home-field] generation queue printer exposes local-source defaults without fallback dirt', () => {
  ...
  assert.match(result.stdout, /summary: .*next pasted prompt must stay queue-only/);
  assert.match(result.stdout, /copyable queue-only prompt:/);
  assert.match(result.stdout, /run `npm run game:home-field:generation-queue -- --id=thalla-stage1-chibi-proof` and use the printed queue results to generate what is needed/);
...
test('[home-field] chibi proof preflight blocks a source matching queue sourceGate hash', () => {
  ...
  return withPreservedFile(homeFieldGenerationQueuePath, () => {
    try {
      ...
      assert.match(result.stderr, /replace the source or add a documented repair method first/);
