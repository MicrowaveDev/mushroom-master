...
For Home Field chibi production-run prompts and queue work, do not collapse "the queue blocked correctly" into "the production run succeeded." If a user asks for another production-ready run while `home-field-generation-queue.json` reports a blocked `sourceGate`, treat the production objective as still unmet: do not call the clean block healthy production output, and do not rerun the blocked source hash. The next pasted prompt must stay queue-only: tell the agent to run `npm run game:home-field:generation-queue -- --id=thalla-stage1-chibi-proof` and use the printed queue `SourceGate recovery production attempt` results to generate what is needed. Put method-change details in the queue output, not the pasted prompt; the runner must follow the printed `sourceGateRecovery` section to create or adopt a new complete `8x4` local source or an explicit documented repair method, then validate that new source through preflight, archive, stage, palette audits, split, candidate, evidence, preview, and verdict. Do not adopt any source or candidate hash listed in `sourceGateRecovery.exhaustedRepairSources`; this includes user-rejected style sources such as old mushroom monk, beige mascot pawn, elderly gnome, faceless mushroom-token, skull-mask, hollow pin-dot-eye, blank-mask, detached motion/squiggle-mark, cute-but-generic anime, or tiny uniform mascot/token reads. The next Thalla source must read as a youthful little-girl mushroom-elf chibi like the liked reference, with compact visible oval/almond doll eyes and rounded cheeks, even when mechanical validators pass. It must not pass merely because it is cute, mechanically clean, or better than older rejected lineages: fail large glossy/white anime eyes, visible hair/bangs/wig fringe under the mushroom cap, brooch/medallion/clasp-like chest ornament, a polished high-res anime turnaround normalized down to `512x256`, or a repaired sheet that collapses into a tiny mascot/token instead of the old liked soft little-girl field-sprite style. It must also stay character-only: no detached motion/action lines, squiggle marks, speed lines, punctuation-like accents, text marks, stray specks, baked shadows, floor/background pieces, or other non-character components outside the main sprite body. For charm-improvement runs, compare the raw generated source, repaired source or candidate, and positive references; palette repair must preserve old liked reference softness and must not flatten Thalla into a hard toy, flat icon, or generic mascot. Do not flip failed visual checks to pass or upgrade `needs_regen` to `needs_review` from mechanical validators, palette/alpha/readability passes, or self-authored `needs_review` prose; clear those only after an absolute positive-reference visual comparison. If only exhausted repair sources exist, report missing fresh authored-source capability instead of rerunning gates or committing the same `needs_regen` verdict. Final handoff must clearly say whether production-ready PNGs were actually produced; a correct blocker report is not production readiness.

For season rank emblem and run-achievement bitmap generation or review, use [docs/season-image-todolist.md](/Users/microwavedev/workspace/mushroom-master/docs/season-image-todolist.md) and [docs/season-image-style-prompt.md](/Users/microwavedev/workspace/mushroom-master/docs/season-image-style-prompt.md) as the workflow source. Production season/rank/achievement bitmap artwork must be created with the imagegen skill/API from the deterministic prompts printed by `npm run game:season:next`; do not author, template, or regenerate these production PNGs with SVG, HTML/CSS, canvas, Puppeteer screenshots, hardcoded glyph paths, procedural medallion drawings, or other deterministic drawing scripts. Deterministic scripts are allowed only to select entries, print prompts, process imagegen raw exports, validate coverage/freshness, build review sheets, and write approved provenance. The pipeline mirrors the artifact one — same `next` / `validate` / `sheet` / `provenance:generate` / `provenance:check` shape under the `npm run game:season:*` aliases, same workspace layout (`.agent/season-image-workspace/{raw,processed,review}/`, gitignored), same shared toolkit (`app/scripts/lib/bitmap-image-toolkit.js`). Production output lives in `web/public/season-ranks/` (4 rank emblems) and `web/public/achievements/` (27 badges — 9 general + 18 character). Until every entry has approved provenance metadata in `app/shared/season-image-metadata.json`, the runtime treats missing PNG/provenance as incomplete production work; runtime fallbacks, if reintroduced, are placeholders only and never bitmap-generation sources. Once `npm run game:season:provenance:check` reports the full current entry count, drop any SVG/glyph fallbacks per the backlog in [`.agent/tasks/season-image-system/spec.md`](/Users/microwavedev/workspace/mushroom-master/.agent/tasks/season-image-system/spec.md). When the user asks for "all season images", "all rank/achievement images", or to see the full season image set, regenerate the deterministic contact sheet via `npm run game:season:sheet` and show `.agent/season-image-workspace/review/contact-sheet.png`; if that command reports byte-identical output, update the underlying PNGs instead of claiming the sheet changed.

For artifact bitmap generation or review, use [docs/artifact-bitmap-todolist.md](/Users/microwavedev/workspace/mushroom-master/docs/artifact-bitmap-todolist.md) as the workflow source. Keep raw imagegen exports, intermediate processed candidates, and generated review sheets under the ignored local workspace `.agent/artifact-image-workspace/` (`raw/`, `processed/`, `review/`); keep only app-facing optimized bitmaps under `web/public/artifacts/`. The all-artifacts review bitmap must be generated with `npm run game:artifacts:sheet`; it is deterministic, sectioned, sorted by artifact ID within each section, and uses the current PNG files plus visual classification metadata. When the user asks for "all artifacts image", "all artifacts images", or to see/give the full artifact image set, regenerate this official sheet first and then show the freshly generated `.agent/artifact-image-workspace/review/contact-sheet.png`; if that command reports a byte-identical output, update the underlying artifact PNGs instead of claiming the sheet changed. The sheet command writes `.agent/artifact-image-workspace/review/contact-sheet.manifest.json` with every input PNG hash, the output hash, and changed artifact IDs; use `--highlight-changed` for a changed-tile visual review and `--allow-unchanged` only when an identical rebuild is intentional. After using imagegen for artifact PNGs, do not accept coverage validation alone: `npm run game:artifacts:validate -- <artifact_id...>` checks by default that app-facing PNGs are newer than raw per-artifact sources under `.agent/artifact-image-workspace/raw/`; add `--fresh-after <start-time-or-source-path>` when validating against a batch start marker. Keep cropping, chroma-key removal, and footprint fitting until the validator no longer reports stale app-facing PNGs, low coverage, low occupied-cell coverage, edge clipping, or unbalanced centered-artifact margins. If an artifact appears cut at a canvas edge, inspect the app PNG alpha margins and the raw imagegen source before retrying: rerun raw-to-bitmap conversion only when the raw contains complete object pixels with enough background; if the raw itself is clipped or touches the edge, regenerate/reselect an uncut raw source or restore an approved archive candidate. The chroma-key helper should reject clipped raw sources by default; use `--allow-clipped-source` only for explicit diagnostics, never production sign-off. Irregular bag masks are placement rules, not PNG clipping stencils: do not mechanically cut empty cells just to make validation pass, and do not use simplified fallback drawings when a good imagegen source exists. The chroma-key helper preserves visual overhang by default for irregular bags; `--organic-mask`, `--force-cell-mask`, and validator `--strict-mask-transparency` are only for intentional diagnostics/placeholders. The contact sheet must also pass `npm run game:artifacts:sheet -- --validate-only`; it should mirror the placed-game model at a fixed review scale (`50px` cells, `8px` gap): only the artifact's footprint cells render behind one uninterrupted bitmap overlay, empty mask cells stay hidden, the bitmap layers above the cells, and each tile reserves only the compact stage height needed for its own footprint. Do not reintroduce a global max-footprint preview stage that adds whitespace to small artifacts. Irregular bag art must not show rectangular cutouts from empty mask cells. Do not use ad hoc `.agent` contact sheets or generator-made collages as sign-off evidence for the full artifact set.

For fusion-catalog expansions, do not final-handoff while new fusion-only artifacts are still using proxy art unless the user explicitly accepts a partial gameplay-only handoff. The production gate is `npm run game:artifacts:release-check`; it bundles missing-art checks, provenance, full artifact coverage, contact-sheet validation, thumbnail review, `npm run game:fusions:check`, production build, and e2e. Use `npm run game:fusions:check` after changing recipes or fusion-only artifacts so stale hard-coded recipe counts, missing result PNGs, duplicate recipes, and ineligible ingredients fail before the long e2e pass. Fusion UI tests should import `tests/game/fusion-catalog-helpers.js` for expected recipe/grid counts instead of hard-coding catalog sizes.
...
- If the user asks to review E2E screenshots, open the corresponding `.json` sidecar files from `captureScreenshot` first. Treat any non-empty `brokenImages` array, `overflowX: true`, unexpected non-zero `document.scrollY` on a newly entered screen, or raw backend-like status token visible in a localized UI as an actionable visual regression even when the Playwright command passed; then inspect the PNG for layout, cropping, overlap, and missing expected elements.
...
- **Desktop prep columns must be visually coherent, not merely present.** When reviewing or asserting the desktop prep screenshot, treat it as broken if the shop column floats above/below the backpack+inventory column, if a large empty left or right column gap appears, if the HUD wedges between the columns, or if the column top edges are not aligned within a small visual tolerance. The intended desktop read is: round heading, HUD, then one cohesive two-column workspace with backpack+inventory on the left and shop+sell zone on the right, followed by actions.
...
- **Prefer backend env-var overrides over direct SQL when a test needs elevated game state.** If the backend exposes a scalar multiplier or override that produces the required state through normal code paths (e.g. `REWARD_MULTIPLIER` to earn mycelium), use it instead of `UPDATE player_mushrooms SET mycelium = N`. Direct SQL bypasses the earning pipeline entirely — bugs in that pipeline are then invisible to the test. The env-var path tests real behavior and keeps setup readable. Reserve raw SQL writes for state that has no env-var equivalent (e.g. setting `active_portrait` when the service call `switchPortrait` is itself what's under test). See `tests/game/helpers.js:earnMycelium` as the canonical example.
