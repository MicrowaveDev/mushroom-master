...
For Home Field chibi production-run prompts and queue work, do not collapse "the queue blocked correctly" into "the production run succeeded." If a user asks for another production-ready run while `home-field-generation-queue.json` reports a blocked `sourceGate`, treat the production objective as still unmet: do not emit the minimal queue launcher by itself, do not call the clean block healthy production output, and do not rerun the blocked source hash. Instead, use the queue's printed `sourceGateRecovery` method-change prompt or write an equivalent prompt that creates or adopts a new complete `8x4` local source or an explicit documented repair method, then validates that new source through preflight, archive, stage, palette audits, split, candidate, evidence, preview, and verdict. Final handoff must clearly say whether production-ready PNGs were actually produced; a correct blocker report is not production readiness.
