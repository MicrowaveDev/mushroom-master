...
const messages = {
  ru: {
    ...
    selectCell: 'Тапни на артефакт, затем на клетку для установки',
  ...
  en: {
    ...
    selectCell: 'Tap an artifact, then tap a board cell to place it',
...
const App = {
  setup() {
    const state = reactive({
      ...
      bootstrap: null,
      appConfig: { localAiLabEnabled: false },
      authCode: null,
      loading: true,
      error: '',
    ...
    async function refreshBootstrap() {
      try {
        ...
        state.appConfig = { localAiLabEnabled: false };
      }
      if (!state.sessionKey) {
        state.loading = false;
        return;
    ...
    function placeArtifact(x, y) {
      ...
      if (x + artifact.width > 4 || y + artifact.height > 4) {
      ...
      next.push(candidate);
      state.builderItems = next.slice(0, 3);
      state.error = '';
    }
  ...
  template: `
    <div class="shell">
      ...
      <section v-else-if="!state.sessionKey" class="panel stack">
        ...
        <button v-if="isLocalLabEnabled" class="ghost" @click="loginViaDevSession">{{ t.authDev }}</button>
      ...
      <template v-else>
        ...
        <section v-else-if="state.screen === 'home'" class="dashboard">
          ...
          <article class="panel">
            ...
            <ul class="chip-list">
              <li v-for="item in state.builderItems" :key="item.artifactId" class="chip">{{ item.artifactId }}</li>
            </ul>
          </article>
        </section>
          ...
          <article class="panel">
            ...
            <div class="artifact-list">
              <button
                ...
                :key="artifact.id"
                class="artifact-btn"
                :class="{ selected: state.selectedArtifactId === artifact.id }"
                @click="state.selectedArtifactId = artifact.id"
              >
          ...
          <article class="panel">
            <div class="board">
              <button
                ...
                :key="cell"
                class="cell"
                @click="placeArtifact((cell - 1) % 4, Math.floor((cell - 1) / 4))"
              >
                <span>{{ buildOccupancy(state.builderItems).get(((cell - 1) % 4) + ':' + Math.floor((cell - 1) / 4)) || '' }}</span>
              </button>
            </div>
            <div class="chip-list">
              <button v-for="item in state.builderItems" :key="item.artifactId" class="chip chip-button" @click="removeArtifact(item.artifactId)">
                {{ item.artifactId }}
              </button>
            </div>
