...
Use sub-agents if available. Keep roles separate: Orchestrator, Prompt/Contract Reviewer, Imagegen Worker, Producer/Validation Worker, Visual Critic.

Prompt queue:
npm run game:home-field:rerun-grass-field

Generate only grass_base_01, grass_base_02, and grass_flowers_01. Save raw files only to the printed .agent/home-field-workspace/raw/ sourcePath values, then run the printed produce commands.
...
Run validation, contact sheet, adjacency sheet, and preview screenshot checks required by docs/home-field-agent-flow.md. Update only those three rows in docs/home-field-asset-review.json, including sceneFitCheck. Do not approve or set accepted:true without explicit human approval.
