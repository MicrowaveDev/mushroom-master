...
10. Produce with `npm run game:home-field:produce-chibi-candidate -- thalla --resize --chroma-key=#ff00ff`. Do not use `--resize-nearest` for production candidates; it preserves hard pixel stair-steps and repeats the rejected style failure.
11. Validate and build evidence using the candidate root:
