...
5. Run `npm run game:home-field:preflight-chibi-proof` before moving or deleting any stale Thalla files. If it fails, stop without cleanup. The run must have a proven way to save actual PNG files to `.agent/home-field-workspace/`; do not rely on inline-only generated images.
