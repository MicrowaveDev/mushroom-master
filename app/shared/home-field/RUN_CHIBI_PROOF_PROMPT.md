...
5. Before any image generation, move or delete stale rejected Thalla raw frames from `.agent/home-field-workspace/raw/thalla_chibi.frame_*.source.png` and stale Thalla candidate output from `.agent/home-field-workspace/candidates/chibi-active-roster/latest/`. Do not reuse the rejected tiny beige/pixel-sprite raw frames.
6. Generate one non-production reference turnaround sheet first and save it to `.agent/home-field-workspace/reference/thalla_chibi_turnaround.reference.png`. It should show Thalla in `down`, `up`, `left`, and `right` directions with the same proportions, top-down 2.5D camera, palette, and detail budget. This reference is for consistency only; do not slice it into final frames.
7. Stop and visually inspect the reference sheet before final frame generation. If the reference sheet already looks like pixel art, a tiny doll sprite, a generic elf, a portrait sticker, a human with a mushroom hat, or a different renderer than the style reference, discard it and regenerate before creating final frames.
8. Generate each final raw frame as a separate isolated transparent PNG into the exact `thalla_chibi.frame_*.source.png` raw paths from the contract. If the run asks for retina/source quality, `128x128` isolated source frames are allowed, but they must downscale cleanly to the current `64x64` runtime sheet. Do not generate final raw frames by cropping a sheet, lineup, grid, scene, or larger multi-sprite image. Do not generate pixel art, a tiny beige doll sprite, a generic elf, or a straight portrait sticker.
9. Produce with `npm run game:home-field:produce-chibi-candidate -- thalla --resize --chroma-key=#ff00ff`. Do not use `--resize-nearest` for production candidates; it preserves hard pixel stair-steps and repeats the rejected style failure.
10. Validate and build evidence using the candidate root:
