...
Use separate sub-agents if available. Generate only `bush_cluster_dark_01`, `bush_cluster_light_01`, and `leaf_sprout_01` as candidate assets. Fix the known issues: no magenta/pink alpha fringe, and make the bushes feel more natural: one irregular asymmetric shrub mass with uneven scalloped edges, varied leaf-cluster sizes, small negative notches, and offset highlights. Avoid broccoli/cauliflower blobs, flower rosettes, evenly spaced circular clumps, repeated stamp marks, or a mound made from many same-size round bushes. Keep `leaf_sprout_01` larger, chunkier, readable at mobile scale, with a warm dark outline and compact shadow.
