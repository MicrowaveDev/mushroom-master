...
For Thalla, first run `npm run game:home-field:generation-queue -- --id=thalla-stage1-chibi-proof`. Use the explicit CLI/API route by default: run `npm run game:home-field:preflight-chibi-proof -- --env-file=<explicit-env-file>` and stop if it fails, except for the one diagnostic built-in output probe allowed by `docs/home-field-imagegen-requirements.md` only when reference-image input binding is already confirmed and disk output is the remaining blocker. Do not infer `.env`; the launcher must provide an env file containing `OPENAI_API_KEY`. Only after preflight passes, archive stale rejected raw/reference/candidate files with `npm run game:home-field:archive-stale-chibi-proof -- thalla --env-file=<explicit-env-file>`; do not delete or reuse them. Then generate a non-production sprite-box reference using `npm run game:home-field:chibi-reference-api-proof -- --env-file=<explicit-env-file>`, not another text-only reference attempt, `view_image`-only attempt, or large painterly turnaround; claim built-in output with `npm run game:home-field:claim-imagegen-output` only for a confirmed built-in path. Visually reject the reference if it misses the style. Generate the coherent grouped `8x4` state sheet only through a reference-capable image path with the passed reference PNG attached as an actual image input, then split it into final raw frames. Idle bob and walk poses must be in the grouped sheet itself, not synthesized after split. Use a limited sprite palette: prefer `12-18` artist-visible colors and stay under `20`, excluding transparency and chroma-key. Use smooth `--resize`, not `--resize-nearest`, when producing the candidate. Mechanical readability does not count as style approval.
