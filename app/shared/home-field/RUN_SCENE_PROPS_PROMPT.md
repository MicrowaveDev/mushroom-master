...
```text
In /Users/microwavedev/workspace/microwave-hub/mushroom-master, run the Home Field scene props candidate batch with sub-agents, exactly as documented in app/shared/home-field/RUN_SCENE_PROPS_PROMPT.md. Generate only: mushroom_cluster_small_amber, mushroom_cluster_small_violet, mushroom_cap_red_spotted, fallen_branch_mycelium. Candidate-only, no app overwrite, validation/screenshots/final links required.
```
