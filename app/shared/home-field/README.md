...
Next scene-readiness candidate batch: after the light bush is no longer attention-grabbing, generate a small object-layer framing batch with `mushroom_cluster_small_amber`, `mushroom_cluster_small_violet`, `mushroom_cap_red_spotted`, and `fallen_branch_mycelium`. These add field-edge personality and scale cues without touching terrain, exits, effects, or chibi sprites. Treat them as tiny field tokens: in the current preview they render around 52x52 CSS pixels on mobile and around 90x90 CSS pixels on desktop, so use one bold silhouette, 2-3 main color regions, and very few large accents. Use the same candidate-only root, `--check-alpha-halo`, `--check-readability` where metadata exists, contact sheet, mobile-readability sheet, and object candidate preview screenshots.
