...
| **archive-stale-chibi-proof** | `npm run game:home-field:archive-stale-chibi-proof -- thalla` | Reruns chibi preflight with the current environment, then moves only stale Thalla reference/raw/candidate proof paths into `.agent/home-field-workspace/rejected/` and prints a manifest. |
| **palette-audit** | `npm run game:home-field:palette-audit -- <png> --out=<json> --swatch=<png>` | Diagnostic chibi palette report. Excludes alpha/#ff00ff, writes JSON plus a swatch PNG, and reports PASS/WARN/FAIL for the `<20` visible design-color budget. For Thalla candidate evidence, run it on the reference sheet, grouped state sheet, and candidate spritesheet using the filenames printed by `game:home-field:next-chibi-proof`. |
...
| **recover-chibi-alpha** | `npm run game:home-field:recover-chibi-alpha -- thalla` | Iterates bounded chroma-key tolerances, reruns split/produce/verifier steps, and stops only when chibi alpha/halo validation passes. Use only for recoverable chroma fringe. |
| **mobile-readability-sheet** | `npm run game:home-field:mobile-readability-sheet -- --ids=bush_cluster_dark_01,bush_cluster_light_01,leaf_sprout_01` | Shows selected props at 128/96/64/48/32px on field-colored backgrounds, writing `.agent/home-field-workspace/review/mobile-readability-sheet.png`. Use `HOME_FIELD_ASSET_ROOT` for candidate roots. |
...
| **record-chibi-verdict** | `npm run game:home-field:record-chibi-verdict -- thalla --verdict=needs_regen --reason-file=<file>` | Copies current Thalla candidate/source/screenshot hashes from `candidate-evidence.manifest.json` into `docs/home-field-asset-review.json` while keeping `accepted: false`. |
...
Current next candidate recommendation: rerun only `bush_cluster_light_01` if polishing foliage before widening the set. Keep its accepted shape direction (few large overlapping masses), but darken and desaturate it so it reads as quiet moss-green instead of bright yellow in the mobile field. `bush_cluster_dark_01` and `leaf_sprout_01` are viable `needs_review` candidates; do not regenerate them unless human review asks for changes.

Current scene-readiness candidate recommendation: rerun only `mushroom_cluster_small_violet`. The latest scene-prop batch left `mushroom_cluster_small_amber`, `mushroom_cap_red_spotted`, and `fallen_branch_mycelium` as viable `needs_review` candidates pending human approval, but the violet cluster reads gray/low-identity at mobile scale. Treat it as a tiny field token: in the current preview it renders around 52x52 CSS pixels on mobile and around 90x90 CSS pixels on desktop, so use one bold silhouette, 2-3 main color regions, and very few large accents. Use the same candidate-only root, `--check-alpha-halo`, `--check-readability` where metadata exists, contact sheet, mobile-readability sheet, and object candidate preview screenshots.
...
Chibi quality must meet the approved prop bar in the composed field preview: crisp warm dark outline, stronger face/cap/robe contrast, finished hand-drawn polish, and no soft beige/downscaled sticker read. Run `--check-chibi-quality` as an objective early gate, but final approval still depends on the chibi looking at least as finished as the approved bushes, mushrooms, and gates in mobile and desktop previews.
...
- **Chibi rows misordered**: row order is locked to `down, up, left, right` top to bottom. The validator enforces this. Idle frames are columns 0-1 and must be a non-duplicated normal-to-little-bob pair, with frame 1 staying upright and moving only a few pixels; walk frames per row are columns 2-7.
