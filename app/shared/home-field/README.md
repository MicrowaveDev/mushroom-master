...
Per-character chibis (not the placeholder) require the full `8 × 4` grid: `2` idle frames plus `6` simple walk frames for each of `down`, `up`, `left`, and `right`. Idle frame `0` is the normal planted pose; idle frame `1` is a tiny squat/squish pose that loops back to normal. Runtime chibi frames are character-only transparent sprites; ground shadows are supplied by the separate shared `chibi_shadow` layer. For the current Thalla proof, use `npm run game:home-field:chibi-proof-context` and `npm run game:home-field:verify-chibi-proof-files` before producing a candidate sheet.
