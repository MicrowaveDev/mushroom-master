...
function extractReferencePrompt() {
  const result = run('npm', ['run', 'game:home-field:next-chibi-proof'], { capture: true });
  const match = result.stdout.match(/## Copyable Sprite-Box Reference Prompt[\s\S]*?```text\n([\s\S]*?)\n```/);
  if (!match) {
    throw new Error('Could not extract Copyable Sprite-Box Reference Prompt from next-chibi-proof output');
  }
