...
function main() {
  ...
  if (chibiCandidate) {
    console.log('  0. Run `npm run game:home-field:generation-queue -- --id=thalla-stage1-chibi-proof`, `npm run game:home-field:chibi-proof-context`, this read-only `npm run game:home-field:next-chibi-proof` helper, then `npm run game:home-field:preflight-chibi-proof -- --env-file=<explicit-env-file>` for the default CLI/API route; use `HOME_FIELD_BUILTIN_IMAGEGEN_CAN_SAVE=1 HOME_FIELD_BUILTIN_IMAGEGEN_CAN_USE_REFERENCES=1 npm run game:home-field:preflight-chibi-proof` only when the launcher/user explicitly confirmed built-in disk output plus actual reference-image input binding for this same session. If preflight or the method gate fails, stop before stale-file archive/imagegen but include this helper output in the blocker report.');
    ...
    console.log('  3. Run `npm run game:home-field:archive-stale-chibi-proof -- thalla --env-file=<explicit-env-file>` with the same preflight-passing environment for CLI/API runs.');
    console.log('  4. Attach the checked-in Thalla reference PNGs as actual image inputs, then use the exact copyable Sprite-Box Reference Prompt for the reference sheet; for the explicit CLI/API path use `npm run game:home-field:chibi-reference-api-proof -- --env-file=<explicit-env-file>` so env loading, SDK setup, exact prompt extraction, API-source normalization, verifier, and palette audit happen serially. Stop at the visual reference gate if palette, style, or status-ornament drift appears. Do not run another text-only reference attempt. If two exact-prompt image-guided reference attempts fail the same visual gate, stop and report instead of burning more blind retries.');
    console.log('  5. Use the explicit CLI/API helper path by default; use built-in imagegen for proof art only after same-agent file output and reference-image input binding are confirmed, or use supplied local proof source PNG inputs. docs/reference PNGs are style references, not proof source inputs. GPT-image-2 CLI/API reference output may be normalized from a required API size such as 1024x768 to the 512x384 sprite-box source before verifier/audit, but palette audit must still pass. Save each generated PNG to the required repo path or claim it with `npm run game:home-field:claim-imagegen-output`.');
  } else {
