...
function chibiLocalSourceDetails(asset) {
  ...
  const opening = sourceBlocked
      ? [
        ...
        'If the user asked for another production-ready run, use the queue output section named SourceGate recovery production attempt instead of this blocked helper as the copyable prompt; a clean blocker is not production-ready output.',
...
function formatAssetPrompt({ asset, promptEntry, anchor, idx, total, fieldContext = false, objectCandidate = false, chibiCandidate = false, terrainCandidate = false, showFallbacks = false }) {
  ...
  if (chibiCandidate && asset.type === 'character') {
    ...
    if (sourceBlocked) {
      ...
      lines.push('For another production-ready run, use the queue SourceGate recovery production attempt prompt; do not hand off this blocked helper as a production launcher.');
...
function main() {
  ...
  if (chibiCandidate) {
    ...
    if (chibiSourceBlocked) {
      ...
      console.log(`  1. Use the prompt block below only as next-source requirements. If the user asked for another production-ready run, use the queue ${recovery?.mode || 'SourceGate recovery production attempt'} prompt instead of this blocked helper. Continue only after replacing the queue sourcePath with a new complete 8x4 local state sheet or adding a documented repair method with evidence.`);
