...
function chibiReferenceTurnaroundPromptBlock(asset, { chibiCandidate = false, showFallbacks = false } = {}) {
  ...
  if (isThallaChibiLocalSourceCandidate(asset, { chibiCandidate }) && !showFallbacks) {
    return [
      ...
      '',
      'Review target: the staged 8x4 sheet should preserve the compact grouped-sheet charm while satisfying the current proof contract: 12-18 artist-visible colors, fewer than 20 total design colors excluding transparency and #ff00ff, compact visible dark oval/almond doll eyes, cap-as-biology, no visible hair bangs or wig fringe, no royal/status ornament clusters, no skull-mask or hollow pin-dot face, no baked foot ovals or ground shadows, no pixel-art or flat-vector overcorrection, no charm-destroying quantization, no hard toy-like palette repair, and no large anime/fashion/showcase proportions. If palette repair is used, the final verdict must say whether charm survived palette repair.',
      '',
