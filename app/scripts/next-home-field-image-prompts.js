...
function chibiLocalSourceMode() {
  try {
    ...
    const item = (queue.items || []).find((entry) => entry.id === CHIBI_QUEUE_ID);
...
function formatAssetPrompt({ asset, promptEntry, anchor, idx, total, fieldContext = false, objectCandidate = false, chibiCandidate = false, terrainCandidate = false }) {
  ...
  if (chibiCandidate && asset.type === 'character') {
    ...
    if (localSource?.sourcePath) {
      ...
      lines.push(`Supplied local state-sheet command: ${localSource.stageCommand}; this copies the queue sourcePath here and derives the reference proxy. HOME_FIELD_CHIBI_LOCAL_IMAGE_INPUTS is legacy override only for a different user-supplied complete 8x4 local PNG.`);
    } else {
