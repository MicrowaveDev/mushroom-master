...
function formatAssetPrompt({ asset, promptEntry, anchor, idx, total, fieldContext = false, objectCandidate = false, chibiCandidate = false, terrainCandidate = false }) {
  ...
  if (chibiCandidate && asset.type === 'character') {
    ...
    lines.push('Supplied local state-sheet path: when HOME_FIELD_CHIBI_LOCAL_IMAGE_INPUTS names one complete 8x4 local PNG outside docs/reference, run npm run game:home-field:stage-chibi-local-source to copy it here and derive the reference proxy; do not run reference imagegen for that local-source path.');
  ...
  if (chibiCandidate && asset.type === 'character') {
    lines.push('For a supplied complete 8x4 local state-sheet source, first run: npm run game:home-field:stage-chibi-local-source');
...
function main() {
  ...
  if (chibiCandidate) {
    ...
    console.log('  5. Use built-in/imagegen skill output as the normal proof-art path after same-agent file output and reference-image input binding are confirmed and the method gate allows it, or use supplied local proof source PNG inputs. For exactly one supplied complete 8x4 local state-sheet PNG in HOME_FIELD_CHIBI_LOCAL_IMAGE_INPUTS, run `npm run game:home-field:stage-chibi-local-source`, skip reference imagegen, verify/audit the derived reference proxy, then continue at staged state-sheet verification/split. Keep that same `HOME_FIELD_CHIBI_LOCAL_IMAGE_INPUTS` value for preflight/archive/stage; this is a supplied complete 8x4 local state-sheet source run. docs/reference PNGs are style references, not proof source inputs. GPT-image-2 API fallback reference output may be normalized from a required API size such as 1024x768 to the 512x384 sprite-box source before verifier/audit, but palette audit must still pass. Save each generated PNG to the required repo path or claim it with `npm run game:home-field:claim-imagegen-output`.');
  } else {
