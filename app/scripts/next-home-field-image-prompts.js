...
function chibiReferenceTurnaroundPromptBlock(asset, { chibiCandidate = false } = {}) {
  ...
  return [
    ...
    '',
    'Method gate after rollout codex-019f1a6c-3143-7631-b3a4-73da0f052070: do not run ad hoc or unchanged built-in sprite-box imagegen attempts. The queue-backed built-in path printed by `npm run game:home-field:generation-queue -- --id=thalla-stage1-chibi-proof` is the current allowed method change because it requires same-context view_image staging of all three reference PNGs, a same-context built-in image_gen call that explicitly uses the visible images as references, and save/claim handoff. Use that queued path for one fresh reference-attempt batch only. If it is unavailable, or if two exact-prompt image-guided attempts through it fail the same visual gate, stop and report this blocker. The checked-in docs/reference PNGs are style references, not supplied proof source PNGs.',
    '',
    '```text',
...
function main() {
  ...
  if (chibiCandidate) {
    ...
    console.log('  4. Attach or stage the checked-in Thalla reference PNGs as actual same-context image inputs, then use the exact copyable Sprite-Box Reference Prompt for the reference sheet. Prefer built-in/imagegen skill output when it can save PNGs and use those references; for built-in local files, view_image is the staging step, not the completed reference proof. The queue-backed built-in same-context staging path is the current allowed method change for one fresh reference-attempt batch after rollout codex-019f1a6c-3143-7631-b3a4-73da0f052070. Use `npm run game:home-field:chibi-reference-api-proof -- --env-file=<explicit-env-file>` only as the paid API fallback after HOME_FIELD_IMAGEGEN_SKILL_UNAVAILABLE=1 is set, so env loading, SDK setup, exact prompt extraction, API-source normalization, verifier, and palette audit happen serially. Stop at the visual reference gate if palette, style, or status-ornament drift appears. Do not run another text-only reference attempt. If two exact-prompt image-guided reference attempts fail the same visual gate, stop and report instead of burning more blind retries.');
