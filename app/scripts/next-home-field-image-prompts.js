...
const GENERATION_QUEUE_PATH = path.join(sharedDir, 'home-field-generation-queue.json');
const CHIBI_QUEUE_ID = 'thalla-stage1-chibi-proof';
...
const BLOCKED_LOCAL_SOURCE_PROMPT_MARKER = 'Blocked local-source chibi proof: do not produce a candidate from the current source hash; use this output only for blocker review and next-source requirements.';
...
function chibiQueueItem() {
  try {
    const queue = loadJson(GENERATION_QUEUE_PATH);
    return (queue.items || []).find((entry) => entry.id === CHIBI_QUEUE_ID) || null;
  } catch {
    return null;
  }
}
...
function chibiLocalSourceMode() {
  return chibiQueueItem()?.generationContract?.stateSheet?.localSourceMode || null;
}

function chibiSourceGate() {
  return chibiQueueItem()?.sourceGate || null;
}

function chibiSourceGateRecovery() {
...
function chibiLocalSourceDetails(asset) {
  ...
  const sourceGate = chibiSourceGate();
  ...
  return [
    ...opening,
    'Treat docs/reference/home-field/chibi-thalla-previous-best-2026-06-26-state-sheet.png, docs/reference/home-field/chibi-thalla-liked-2026-06-23.png, and docs/reference/home-field/chibi-style-agent-log-reference.png as styleReferences for visual review only. The liked Thalla reference is the primary positive style target for the next source\'s youthful little-girl face/body read: cute dark storybook warmth, a simple BJD-inspired chibi doll face, smooth BJD-like face planes, rounded cheeks, sweet simple expression, compact visible oval/almond doll eyes, tiny childlike body, and soft hand-drawn field-sprite charm. They are not proof sources and must not be attached to imagegen in this run.',
    'The grouped state sheet must contain the idle bob and walk poses. Row order must be down, up, left, right. Columns 0-1 are idle frames: col 0 normal planted pose, col 1 little 1-3px bob/squish that loops back to normal while staying upright; do not make a crouch, seated pose, or deep squat. Columns 2-7 are the walk lane: aim for 4 meaningful walk poses distributed across 6 slots, with optional subtle holds/in-betweens rather than six important poses. Every frame must stay character-only with no detached motion/action lines, squiggle marks, speed lines, punctuation-like accents, text marks, or stray specks outside the main sprite body.',
    'Follow docs/home-field-imagegen-requirements.md, docs/home-field-chibi-candidate-contract.md, and docs/home-field-chibi-style-reference.md. Preserve Thalla as an ancient gold-white mushroom-elf field-sprite leader with cap-as-biology, not a hair-covered hat, compact visible dark oval/almond doll eyes with fiery-gold life, 1-2 flat gold mycelium/spore marks total, simple fungal robe/cap silhouette, warm bone/gold/white/brown palette, and calm biostasis stillness. For this chibi surface, ancient/leader identity must still read as a little-girl mushroom-elf field sprite like the liked reference, not as an elderly monk, elderly gnome, faceless mushroom token, skull-mask face, hollow pin-dot eyes, blank mask face, or generic mascot pawn.',
    ...
    'Use a deliberately limited sprite palette: 12-18 artist-visible colors across the source sheet, fewer than 20 total design colors excluding transparency and #ff00ff chroma-key. Reuse shared cap/robe/skin/gold ramps; do not introduce many near-duplicate beige, cream, blush, glow, or gold tones. Use softness through shape language rather than extra colors: pillowy cap silhouette, rounded cheek planes, gentle tiny mouth, curved mitten hands, warm dark outline, and broad two-tone face/cap/robe ramps. The palette budget must not become hard flat cel/vector/pixel art, hard toy-like palette repair, or a generic toy/mascot read. Do not overcorrect the palette rule into hard pixel art, clean vector/cel icon art, a cold exactly-16-swatches exercise, or a large anime/fashion turnaround.',
    'Use an elevated top-down 2.5D hub-sprite camera with the top of the mushroom cap/head visible and the feet/base planted on the map. Quality bar: the composed chibi must look at least as crisp, contrasted, and finished as approved Home Field props; avoid blurry/downscaled sticker quality; use chunky readable shapes, thicker warm-brown outline, clear face/cap/robe value separation, and strong tiny silhouette. Make a small readable hand-drawn chibi with BJD-inspired doll simplicity and the youthful little-girl appeal of the liked 2026-06-23 Thalla reference, not pixel art and not a portrait.',
    'Eyes may be expressive like the liked 2026-06-23 reference, but must stay compact, visible dark oval/almond doll eyes for a map sprite and must not collapse into hollow pin-dots, a skull-mask face, glossy anime eyes, eyelashes, or huge white portrait eyes. Do not bake a blob/cast shadow, foot oval, floor contact patch, grass, floor, text, UI, motion/action lines, squiggle marks, speed lines, punctuation-like marks, stray specks, or per-frame ground shadow into the source sheet; the renderer uses a separate shared chibi_shadow layer.',
    'Mechanical sheet success, alpha success, mobile readability, and chibi-quality validation do not count as style approval. The composed preview must match the hand-drawn elevated 2.5D field-sprite target and must read as youthful little-girl Thalla like the positive references, not old monk, beige mascot pawn, elderly gnome, faceless mushroom token, skull-mask face, hollow pin-dot eyes, or blank mask face. If palette cleanup, quantization, resize, or chroma repair is used, compare the raw generated source against the repaired source and final candidate; reject the repair unless charm survived palette repair and the soft BJD doll face was not flattened. If the supplied source fails the visual gate, record the verdict and require a new supplied complete local source or an intentional future method change; do not silently fall back to imagegen.'
  ].join(' ');
}
...
function promptConstraintsForOutput(asset, promptEntry, { chibiCandidate = false } = {}) {
  ...
  return [
    ...
    'The split output must contain 32 isolated character-only raw frames: 2 idle plus 6 walk-lane frames for each direction, with non-duplicated normal/little-bob idle frames, a 4-pose walk-cycle target across the 6 slots, and at least 3 unique walk frames per direction. Idle frame 1 must be only a little 1-3px bob/squish while staying upright; no crouch, seated pose, or deep squat. The character-only contract also bans detached motion/action lines, squiggle marks, speed lines, punctuation-like accents, text marks, stray specks, or any non-character component outside the main sprite body.',
    'No blurry/downscaled sticker quality, no weak outline, no low-contrast beige-on-beige chibi, no crop-from-reference workflow, no deterministic drawn substitutes reported as fresh art, no baked ground shadow in chibi frames, no human-only mushroom hat, no visible hair bangs or wig fringe under a mushroom cap, no missing elf ears when ears are shown, no front-facing portrait sticker, no straight-on fashion pose, no full scene, no grass background, no text in final raw frames, no UI, no floor plane, no detached motion/action lines, no squiggle marks, no speed lines, no punctuation-like accents, no stray specks, no high-detail portrait rendering, no large soft illustration palette, no more than roughly 20 visible design colors, no painterly gradients, no airbrushed blush, no many local highlight/shadow tones, no near-duplicate beige/gold micro-tones, no overcorrected flat/vector/cel/pixel style, no charm-destroying quantization, no hard toy-like palette repair, no exactly-16-swatches prompt drift, no large anime/fashion/showcase turnaround proportions, no full-height polished character-design figures, no soft painterly turnaround sheet, no elderly monk, no elderly gnome, no faceless mushroom token, no beige mascot pawn, no skull-mask face, no hollow pin-dot eyes, no blank mask face, no adult/elder robe-doll read, no earrings, no jewelry, no royal regalia, no crown jewels, no forehead gems, no brooches, no chest medallions, no pendants, no jewelry-like cap crests, no gold filigree, no scalloped collars, no ornamental robe borders, no decorative trim clusters, no sleeve cuff trim, no clasps, no collar jewels, no repeated gold badges, no baked foot ovals, no ornate filigree, no scattered cap freckles, no many small gold droplets, no particle halo, no jewelry clusters, no tiny chains, no glossy anime eyes, no eyelashes, no huge white portrait eyes, no eye-dominated face, no pure-black outline, no pure-white highlights, no realistic doll-photo rendering, no glossy plastic toy rendering.',
    sourceBlocked
      ? 'Must remain simpler than the 2026-06-20 candidate, with a youthful little-girl BJD-inspired chibi doll read at mobile scale, compact visible oval/almond doll eyes, and remain readable on green field. After sourceGate is cleared, produce with smooth --resize rather than --resize-nearest when source frames are larger than 64x64.'
      : 'Must remain simpler than the 2026-06-20 candidate, with a youthful little-girl BJD-inspired chibi doll read at mobile scale, compact visible oval/almond doll eyes, and remain readable on green field. Produce with smooth --resize rather than --resize-nearest when source frames are larger than 64x64.'
  ].join(' ');
}
...
function formatAssetPrompt({ asset, promptEntry, anchor, idx, total, fieldContext = false, objectCandidate = false, chibiCandidate = false, terrainCandidate = false, showFallbacks = false }) {
  ...
  const sourceBlocked = isThallaChibiLocalSourceCandidate(asset, { chibiCandidate }) && chibiSourceGateBlocked();
  lines.push(sourceBlocked
    ? BLOCKED_LOCAL_SOURCE_PROMPT_MARKER
    : isThallaChibiLocalSourceCandidate(asset, { chibiCandidate })
  ...
  if (chibiCandidate && asset.type === 'character') {
    ...
    lines.push(sourceBlocked
      ? 'Grouped state sheet path (do not stage the blocked source hash here): .agent/home-field-workspace/raw/thalla_chibi.states.source.png'
      : 'Grouped state sheet path (stage supplied local source HERE): .agent/home-field-workspace/raw/thalla_chibi.states.source.png');
    if (localSource?.sourcePath) {
      lines.push(`Supplied local state-sheet sourcePath from queue JSON: ${localSource.sourcePath}`);
      lines.push(sourceBlocked
        ? `Supplied local state-sheet command after sourceGate is cleared: ${localSource.stageCommand}; do not run it for the blocked source hash.`
        : `Supplied local state-sheet command: ${localSource.stageCommand}; this copies the queue sourcePath here and derives the reference proxy.`);
    } else {
      lines.push('Supplied local state-sheet path: use --source=<png> with npm run game:home-field:stage-chibi-local-source to copy one complete 8x4 local PNG outside docs/reference here and derive the reference proxy; do not run reference imagegen for that local-source path.');
    }
  ...
  if (objectCandidate || chibiCandidate || terrainCandidate) {
    lines.push(sourceBlocked
      ? `Candidate output path (not written while sourceGate is blocked): ${candidateRoot}/${asset.outputPath}`
      : `Candidate output path (written by candidate producer): ${candidateRoot}/${asset.outputPath}`);
  }
...
function chibiReferenceTurnaroundPromptBlock(asset, { chibiCandidate = false, showFallbacks = false } = {}) {
  ...
  if (isThallaChibiLocalSourceCandidate(asset, { chibiCandidate }) && !showFallbacks) {
    return [
      ...
      '- `docs/reference/home-field/chibi-thalla-liked-2026-06-23.png` - primary positive youthful little-girl Thalla face, cap, robe, cute dark storybook warmth, simple BJD-inspired chibi doll face, smooth BJD-like face planes, compact oval/almond doll eyes, rounded-cheek appeal, and soft hand-drawn charm; simplify heavily.',
      ...
      '',
      'Review target: the staged 8x4 sheet should preserve the compact grouped-sheet charm while satisfying the current proof contract: 12-18 artist-visible colors, fewer than 20 total design colors excluding transparency and #ff00ff, compact visible dark oval/almond doll eyes with minimal/no white sclera, cap-as-biology, no visible hair bangs or wig fringe, no brooch/medallion/clasp ornament, no royal/status ornament clusters, no skull-mask or hollow pin-dot face, no baked foot ovals or ground shadows, no pixel-art or flat-vector overcorrection, no charm-destroying quantization, no hard toy-like palette repair, no cute-but-generic anime/chibi character-sheet style, and no large anime/fashion/showcase proportions. If palette repair is used, the final verdict must say whether charm survived palette repair.',
      '',
  ...
  return [
    ...
    '- `docs/reference/home-field/chibi-thalla-liked-2026-06-23.png` — primary positive youthful little-girl Thalla face/cap/robe appeal with cute dark storybook warmth, a simple BJD-inspired chibi doll face, smooth BJD-like face planes, rounded cheeks, and compact visible oval/almond doll eyes; simplify heavily.',
    ...
    '',
    'Paste this exact prompt into imagegen for the non-production reference sheet, with those local PNGs attached or same-context staged as actual image inputs. For the built-in path, load each local PNG with view_image only as the current imagegen skill\'s same-context input-staging step, then call built-in image_gen in that same context and explicitly name those visible images as references. Do not hand-compose or add extra style terms. Do not run this as another text-only generation; passive viewing, listing filesystem paths, or mentioning paths in the prompt is not enough. If the imagegen path cannot attach or use the checked-in PNGs as actual same-context image inputs to the generation call, stop and report that image-guided generation is required.',
    '',
    'Method gate after rollout codex-019f1eb1-1027-7752-95cf-d4f37cb0041c: do not run the queue-backed built-in same-context reference-staging path again unchanged. That path used same-context view_image staging of all three reference PNGs, a same-context built-in image_gen call with the visible images named as references, and save/claim handoff, but still produced a 1448x1086 oversized turnaround with blobs up to 249x332 and 91 significant exact colors. The built-in staging method is exhausted for unchanged Thalla proof runs. Continue only with a different reference-capable generation/editing method, supplied local proof source PNGs outside docs/reference, or explicit user-approved fallback; otherwise stop and report this blocker. The checked-in docs/reference PNGs are style references, not supplied proof source PNGs.',
    '',
    '```text',
    'Create one non-production sprite-box reference sheet for Thalla only, for the Mushroom Battles Home Field chibi proof.',
    '',
    'Input images: use the attached checked-in reference images as guidance. Preserve the compact sprite proportions and charm of the previous-best state sheet as the primary layout/scale authority, use the liked Thalla image for cute dark storybook warmth, a simple BJD-inspired chibi doll face, smooth BJD-like face planes, rounded cheeks, simplified cute little-girl face/cap/robe appeal with compact visible oval/almond doll eyes, and use the chibi style reference only for field-sprite scale/outline simplicity.',
    '',
    ...
    '',
    'Style target: hand-drawn elevated 2.5D field sprite with BJD-inspired chibi doll simplicity. Squat field-sprite proportions, oversized but not eye-dominated head, tiny grounded body, simple costume blocks, warm dark irregular outline, rounded cheeks, compact visible oval/almond doll eyes, tiny mouth/nose, mitten-like hands, tiny planted feet, visible elf ears when ears are shown, and enough top of the mushroom cap/head visible to belong on the map. Design for 64px readability first: broad shape clusters, visible dark doll eyes with only a tiny gold life glint and minimal/no white sclera, no hollow pin-dot/skull-mask face, no eyelashes or glossy anime eye shine, no painterly surface texture, no enlarged illustration detail, and no merely cute generic anime character-sheet read.',
    '',
    'Thalla identity: ancient gold-white mushroom-elf field-sprite leader; mushroom-elf biology, not a human with a mushroom hat or hair under a cap; the mushroom cap is part of the character biology, not a removable hat; compact visible dark oval/almond doll eyes with fiery-gold life; 1-2 large flat gold mycelium/spore marks total across the whole sprite; simple fungal robe/cap silhouette; warm bone/gold/white/brown palette; calm biostasis stillness. Avoid visual royalty language in the art.',
    '',
    'Status simplification: represent authority through cap silhouette, robe blocks, posture, and 1-2 flat mycelium/spore marks only. Use one plain robe block with no layered cape/collar ornament. No royal regalia, crown jewel, forehead gem, brooch, chest medallion, pendant, jewelry-like cap crest, gold filigree, scalloped collar, ornamental robe border, decorative trim cluster, sleeve cuff trim, clasp, collar jewel, repeated gold badge, or small round chest mark that reads as a brooch/medallion/clasp instead of flat painted mycelium.',
    '',
    'Palette plan: 12-18 artist-visible colors, fewer than 20 total design colors excluding transparency and #ff00ff. Use shared cap/robe/skin/gold ramps instead of many local beige, cream, blush, glow, or gold shades. Use softness through shape language rather than extra colors: pillowy cap silhouette, rounded cheek planes, gentle tiny mouth, curved mitten hands, warm dark outline, and broad two-tone face/cap/robe ramps. Use broad flat clusters and one-step shadows/highlights, not watercolor texture, painterly gradients, airbrushed blush, soft glow, or many near-duplicate tones.',
    '',
    'Preserve direction: prioritize the checked-in 2026-06-26 previous-best Thalla state sheet over generic chibi/anime defaults: keep its squat proportions, cap/body/face charm, coherent state-sheet feel, compact visible oval/almond eyes, rounded cheeks, and cute little-girl expression while fixing palette bloat, ornament, and sticker softness. Use the 2026-06-23 liked Thalla image as the primary positive youthful little-girl face/cap/robe appeal, simplified into the field-sprite read, and preserve that charm through any palette repair. Use the chibi style reference only for proportions, outline weight, BJD-inspired simplicity, and scene-scale simplicity; do not copy characters, costumes, symbols, or composition.',
    '',
    'Hard avoids: no pixel art, no tiny beige doll sprite, no generic elf, no straight portrait sticker, no human with mushroom hat, no visible hair bangs or wig fringe, no cute-but-generic anime/chibi character sheet, no high-res polished turnaround normalized down as proof, no large anime/fashion turnaround, no enlarged showcase turnaround, no quadrant-filling character art, no full-height polished character-design figures, no soft painterly turnaround sheet, no old monk, no elderly gnome, no beige mascot pawn, no faceless mushroom token, no skull-mask face, no hollow pin-dot eyes, no blank mask face, no earrings, no jewelry, no royal regalia, no crown jewel, no forehead gem, no brooch, no chest medallion, no pendant, no jewelry-like cap crest, no scalloped collar, no ornamental robe border, no decorative trim clusters, no sleeve cuff trim, no clasp, no collar jewel, no repeated gold badges, no hard flat cel/vector icon art, no hard toy-like palette repair, no charm-destroying quantization, no cold exactly-16-swatches exercise, no dense cap spots, no scattered gold freckles, no many gold droplets, no ornate filigree, no watercolor/painterly cap texture, no particle halo, no huge white portrait eyes, no large glossy/white anime eyes, no glossy anime eyes, no eyelashes, no eye-dominated face, no realistic doll photo, no glossy toy render, no baked blob/cast shadow, no foot oval, no floor contact patch.',
    '```'
  ].join('\n');
}
...
function main() {
  ...
  const chibiSourceBlocked = chibiCandidate && chibiSourceGateBlocked();
  if (chibiCandidate) {
    const localSource = chibiLocalSourceMode();
    const sourceGate = chibiSourceGate();
    const sourcePath = localSource?.sourcePath || '<queue-localSourceMode.sourcePath>';
  ...
  if (chibiCandidate && chibiSourceBlocked) {
    console.log('  6. Stop here while sourceGate is blocked; do not run split, producer, validation, evidence, preview, or verdict commands for this source hash.');
  ...
  if (objectCandidate || (chibiCandidate && !chibiSourceBlocked) || terrainCandidate) {
  ...
  if (chibiCandidate && chibiSourceBlocked) {
    console.log('  7. Report the blocked source path/hash, failed reference proxy hash, failed palette counts, and that no candidate or app-facing PNG was produced.');
  } else {
