...
function main() {
  ...
  if (chibiCandidate) {
    ...
    console.log('  4. Attach the checked-in Thalla reference PNGs as actual image inputs, then use the exact copyable Sprite-Box Reference Prompt for the reference sheet; for CLI/API fallback prefer `npm run game:home-field:chibi-reference-api-proof -- --env-file=<explicit-env-file>` so env loading, SDK setup, exact prompt extraction, API-source normalization, verifier, and palette audit happen serially. Stop at the visual reference gate if palette, style, or status-ornament drift appears. Do not run another text-only reference attempt. If two exact-prompt image-guided reference attempts fail the same visual gate, stop and report instead of burning more blind retries.');
    console.log('  5. Use built-in imagegen for proof art only after same-agent file output and reference-image input binding are confirmed, or use supplied local proof source PNG inputs / explicit CLI fallback; docs/reference PNGs are style references, not proof source inputs. GPT-image-2 CLI/API reference output may be normalized from a required API size such as 1024x768 to the 512x384 sprite-box source before verifier/audit, but palette audit must still pass. Save each generated PNG to the required repo path or claim it with `npm run game:home-field:claim-imagegen-output`.');
  } else {
