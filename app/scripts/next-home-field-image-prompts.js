...
function main() {
  ...
  if (chibiCandidate) {
    ...
    console.log('  5. Use built-in imagegen for proof art only after same-agent file output and reference-image input binding are confirmed, or use supplied local proof source PNG inputs / explicit CLI fallback; docs/reference PNGs are style references, not proof source inputs. Save each generated PNG to the required repo path or claim it with `npm run game:home-field:claim-imagegen-output`.');
  } else {
