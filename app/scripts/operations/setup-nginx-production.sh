...
Examples:
  ...
  sudo app/scripts/operations/setup-nginx-production.sh --install
  sudo app/scripts/operations/setup-nginx-production.sh --install --tls off
  sudo app/scripts/operations/setup-nginx-production.sh --install --port 3021
...
First-time TLS flow:
  1. sudo app/scripts/operations/setup-nginx-production.sh --install --tls off
  2. sudo certbot certonly --webroot -w /var/www/certbot -d mushroombattles.com -d www.mushroombattles.com
  3. sudo app/scripts/operations/setup-nginx-production.sh --install --tls on
EOF
}
...
render_config() {
  cat <<EOF
# Managed by app/scripts/operations/setup-nginx-production.sh.
