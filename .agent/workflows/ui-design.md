...
- **Eliminate nesting**: avoid borders inside borders, and avoid filled cards inside filled cards. If a parent container has a visible frame (border, background fill, or border-radius), its children must be flat content — no `background`, `border`, or `border-radius` of their own. The parent provides the single frame; children are content blocks separated by whitespace, a thin divider, or typography. This applies to *any* parent–child card relationship: fighter cards inside a panel (`fighter` uses `padding: 0; border: none; background: none`), summary entries inside a summary card (e.g. each side's stats inside `.battle-summary-card`), reward chips inside a results panel, list rows inside a section card. If the inner element needs visual weight, give it stronger typography or a colored value — not a second card surface.
...
- **Digits render from `DigitsSerif` (Source Serif 4), letters from Georgia — keep the stack intact.** The Mini App body font is Georgia, which on every common platform ships with old-style figures only: `1` sits short, `3`/`5`/`7`/`9` have descenders that hang below the baseline of surrounding letters. The user flagged this as visually wrong. Verified non-fixes: `font-variant-numeric: lining-nums` and `font-feature-settings: "lnum"` (Georgia has no lining glyphs to swap to); inflating `line-height` / `padding-bottom` (descender stays put, only adds washed-out airspace); `transform` / `vertical-align` nudges on digits (descender just moves to a different wrong place). The canonical fix in this repo is a digit-scoped serif webfont: **Source Serif 4** (Adobe, open-licensed, variable font covering weights 400–700 in a single 134 KB woff2, ships *only* with lining figures — its woff2 has no `onum` feature and no old-style glyphs at all, so the result cannot regress to old-style regardless of OpenType state). It is self-hosted at `web/public/fonts/source-serif-4.woff2` and registered as the single `@font-face` family `DigitsSerif` (one declaration per weight, both pointing at the same variable file) with `unicode-range: U+0030-0039, U+002B, U+002D, U+002C, U+002E` so it only renders digits and signs. The body font stack starts with `"DigitsSerif"` and falls back to Georgia for everything else. **Important picking constraint when ever changing the digit font**: Google Fonts subsets woff2 files and strips the `lnum`/`onum` figure-style features, so a font whose lining figures live behind an OpenType feature toggle (e.g. PT Serif — already tried and failed for this reason; the subsetted woff2 only retained `ccmp`/`frac`/`liga`/`locl` and rendered old-style figures by default) is not safe. The replacement font must default to lining figures in its cmap, not via a feature. Do not remove `DigitsSerif` from the body stack, do not add `font-family` overrides on stat elements that omit it, and do not delete the woff2 file. A sans-serif equivalent of this hack was previously tried and rejected for this app — the substitute font must remain a warm serif. Keep `font-variant-numeric: lining-nums tabular-nums` on stat elements; `tabular-nums` still gives the equal-width column behavior the layouts depend on (and Source Serif 4's woff2 retains `tnum` even after Google's subsetting).
...
- **Nested card chrome.** An outer card with a border/fill/radius containing inner blocks that *also* have their own `background`, `border`, or `border-radius` reads as data containers inside data containers — a classic dated pattern. Flatten the inner level: drop the inner `background`/`border`/`border-radius`, keep the inner `padding` only as much as the rhythm needs, and separate sibling inner blocks with whitespace or a single thin divider (`border-right` between two columns, `border-bottom` between stacked rows). The single visible frame should be the *outermost* one. Common offender: a "summary" or "details" card with two or more entry rows that each get their own filled tile.
