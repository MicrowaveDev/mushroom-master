...
When a queue item is blocked by `sourceGate` and the user asks for another production-ready run, the queue output must also carry `sourceGateRecovery`: a copyable queue-only launcher plus required actions and success criteria. The recovery prompt must tell the next agent only to run the queue script and use the printed queue results. The method-change instructions belong in the printed queue output, not in the pasted prompt: create or adopt a new complete `8x4` local state-sheet source or an explicit documented repair method, save it under `.agent/home-field-workspace/supplied/`, use `--source=<new-source-png>` through preflight/archive/stage, and only then continue through audits, split, candidate production, evidence, preview, and verdict. A clean blocker report is not production readiness; final handoff must say production-ready PNGs were not produced unless a new candidate exists and passes every listed gate.
