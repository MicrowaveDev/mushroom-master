...
For supplied complete local state-sheet source runs, preflight should print whether the single local PNG is a complete `8x4` sheet with square cells. For queue-backed runs, use the queue-owned source path with `--source` for preflight, archive, and stage. `HOME_FIELD_CHIBI_LOCAL_IMAGE_INPUTS` is legacy override only for a different user-supplied complete local state-sheet PNG; do not put it in the pasted prompt for the current queue source. After preflight and stale-file archive pass, use:
