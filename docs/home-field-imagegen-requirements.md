...
When a queue item is blocked by `sourceGate` and the user asks for another production-ready run, the queue output must also carry `sourceGateRecovery`: a copyable queue-only launcher plus required actions and success criteria. The recovery prompt must tell the next agent only to run the queue script and use the printed queue `SourceGate recovery production attempt` results, not the generic minimal launcher wording. The method-change instructions belong in the printed queue output, not in the pasted prompt: prefer creating or accepting a genuinely new authored complete `8x4` local state-sheet source outside `docs/reference`, save it under `.agent/home-field-workspace/supplied/`, use `--source=<new-source-png>` through preflight/archive/stage, and only then continue through audits, split, candidate production, evidence, preview, and verdict. A palette-cleanup/repair method is secondary: it must be explicit, evidence-backed, and labelled as candidate repair/diagnostic work unless visual review proves it clears the original source defects. When palette cleanup, quantization, resize, or chroma repair is used, visual review must compare the raw generated source, repaired source, final candidate, and positive references; repair is not production readiness if it flattens the old liked reference's cute dark storybook warmth, simple BJD-inspired chibi doll face, smooth BJD-like face planes, rounded cheeks, or cute little-girl charm. Passing palette/count scripts alone is not production readiness, and neither is a source/candidate that passes mechanical gates while reading as an old monk, beige mascot pawn, elderly gnome, faceless mushroom token, skull-mask face, hollow pin-dot-eye mask, blank mask face, hard toy, flat icon, generic mascot, or cute generic anime/chibi character sheet instead of the youthful little-girl Thalla style in the liked reference with visible oval/almond doll eyes and rounded cheeks. A candidate is also not production-ready merely because it is better than older rejected lineages; the visual gate must compare against the positive references as an absolute target. For Thalla recovery, fail large glossy/white anime eyes, visible hair/bangs/wig fringe under the mushroom cap, brooch/medallion/clasp-like chest ornament, or a high-res polished turnaround normalized down to `512x256` without visual style approval. Do not upgrade `needs_regen` to `needs_review`, or flip visual fail checks to pass, from mechanical validators, palette/alpha/readability passes, or self-authored `needs_review` prose. If no replacement source already exists, the runner still must attempt the queued recovery path or report missing source-generation capability explicitly instead of treating the old blocker as completion. A clean blocker report is not production readiness; final handoff must say production-ready PNGs were not produced unless a new candidate exists and passes every listed gate.
...
- one supplied complete `8x4` local Thalla state-sheet source PNG outside `docs/reference/home-field/`, either from the queue JSON `localSourceMode.sourcePath` or an explicit `--source=<png>` override, staged with `npm run game:home-field:stage-chibi-local-source -- --source=<png>` into `.agent/home-field-workspace/raw/thalla_chibi.states.source.png`; the helper derives `.agent/home-field-workspace/reference/thalla_chibi_turnaround.reference.png` as a non-production reference proxy for evidence and does not run imagegen;
...
For supplied complete local state-sheet source runs, preflight should print whether the single local PNG is a complete `8x4` sheet with square cells. For queue-backed runs, use the queue-owned source path with `--source` for preflight, archive, and stage. A different supplied local state-sheet PNG must also use `--source` or a queue source-path update, never an ambient runtime override. After preflight and stale-file archive pass, use:
...
Prompt issuance is source-hash gated. A supplied local `sourcePath` can make the queue ready only until that exact source hash fails a hard verifier, palette audit, or visual gate. After a failed source run, the queue must record a `sourceGate` with rollout, source hash, failed command, evidence counts, and next action; the default queue output must stop issuing a production launcher for that same hash. Preflight must reject a `--source` PNG whose sha256 matches a blocked `sourceGate`, even if the file exists and still has a complete `8x4` layout. Continue only after replacing `generationContract.stateSheet.localSourceMode.sourcePath` with a new authored complete `8x4` local source outside `docs/reference`. A palette-cleanup/repair method is allowed only as an explicit secondary candidate-repair method with manifest evidence and the same visual gates.

The queue must also record any repair lineage that already reached a `needs_regen` verdict, and any recovery source/candidate the user rejected for missing the required style read, in `sourceGateRecovery.exhaustedRepairSources`, including the source hash and candidate hash. This includes candidates that pass mechanical gates but are only relative improvements over older rejected lineages. The default queue printer and `preflight-chibi-proof -- --source=<png>` must reject those exhausted source hashes before archive/stage. A later repair attempt is valid only if it uses a stronger explicit method and produces source/candidate hashes not listed as exhausted. If the only available replacement source is an exhausted repair source, stop and report missing fresh authored-source capability instead of rerunning gates or committing the same verdict.

Do not evaluate a sourceGate-blocked run as "healthy" when the active user request was production-ready PNGs. It can be healthy queue behavior and still be an incomplete production attempt. The correct next handoff is either a blocker report, when the user asked only for diagnosis, or the queue-only `sourceGateRecovery.copyablePrompt`, when the user asked for another production-ready run. The receiving agent must then use the printed queue `SourceGate recovery production attempt` results for the actual new-source or repair-method workflow.

The same default-path rule applies to `npm run game:home-field:next -- --preset=chibi-proof` and `app/shared/home-field/RUN_CHIBI_PROOF_PROMPT.md`: their default Thalla local-source output must not tell the worker to attach `docs/reference` images to imagegen, run `chibi-reference-api-proof`, claim built-in outputs, or regenerate the state sheet. They may show inactive method history only by pointing to the queue `--show-fallbacks` view or, for fallback tooling, by running `npm run game:home-field:next -- --preset=chibi-proof -- --show-fallbacks`.
...
Load fallback credentials through the explicit env file instead of searching neighboring repos, assuming inherited shell state, or guessing `mushroom-master/.env`. The structured queue item records `doNotInferEnvFile: true`; rollout `codex-019f1dbd-e6dd-70e0-a7fe-53977b1cc831` correctly blocked because the guessed `.env` did not provide the required imagegen fallback environment. Future paid API fallback files must contain `OPENAI_IMAGEGEN_API_KEY` and `HOME_FIELD_IMAGEGEN_SKILL_UNAVAILABLE=1`; plain `OPENAI_API_KEY` does not count. The helper keeps the prompt source in `npm run game:home-field:next -- --preset=chibi-proof`, passes the checked-in reference PNGs as actual `--image` inputs, preserves an API-size source PNG, normalizes it to the current `512x384` sprite-box reference when needed, then runs `verify-chibi-proof-files -- --reference` and `palette-audit --fail-on-bloat` serially. A normalized reference still fails if palette bloat, status ornament, hair/wig reads, or other visual gate issues remain.
...
Before running `npm run game:home-field:record-chibi-verdict`, regenerate candidate evidence with `HOME_FIELD_CANDIDATE_ROOT=.agent/home-field-workspace/candidates/chibi-active-roster/latest HOME_FIELD_CANDIDATE_IDS=thalla npm run game:home-field:candidate-evidence`. The verdict helper intentionally refuses the default `.agent/home-field-workspace/review/candidate-evidence.manifest.json` when its `candidateRoot` is not that canonical candidate root or when it contains `tmp/` candidate paths. Prefer `--reason-stdin` or `--reason=<text>` for the visual-critic reason so verdict recording does not depend on an ignored scratch file being created under the correct repo root.
...
If palette cleanup or external palette-aware tools are used, treat them as diagnostic or candidate-repair experiments only. Follow `docs/home-field-chibi-palette-cleanup-research.md`: palette improvement does not approve a chibi that still fails cap biology, eye scale, ornament, source-sprite occupancy, or composed field-sprite style. Palette repair must include pre/post visual comparison of the raw generated source, repaired source, final candidate, and positive references, and the verdict reason must say whether charm survived palette repair. A repaired candidate that preserves blocked-source visual defects or flattens the liked reference's soft doll-face warmth into a hard toy, flat icon, or generic mascot must be recorded as `needs_regen`, even when the palette audits and mechanical validators pass.
