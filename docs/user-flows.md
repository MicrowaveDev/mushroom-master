...
Step 3: Battle Replay (auto-shown after ready)
  ...
  Sees (while replay plays):
    ...
    - Active speech bubble is anchored to the speaking fighter's portrait and reads as character speech, not a detached caption. It must not cover the configured face/head band for that portrait variant; preferred placement is below the face with the tail pointing back toward the head.
  ...
  Expected:
    ...
    - Bubble-review evidence includes both a cast-wide screenshot and deterministic per-character screenshots for every portrait variant, with JSON sidecars. Reviewers should inspect the per-character crops when tuning portrait/bubble config.
