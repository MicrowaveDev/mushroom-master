...
Step 3: Battle Replay (auto-shown after ready)
  ...
  Sees (while replay plays):
    ...
    - Active speech bubble is anchored inside the speaking fighter's portrait and reads as character speech, not a detached caption. It must not cover the configured face/head band or the name overlay; preferred placement is near the head with the tail pointing back toward the speaker.
