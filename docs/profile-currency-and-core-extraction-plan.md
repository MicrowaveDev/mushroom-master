...
Updated again 2026-07-15 after Phase 8BG completion: the bounded residual
raster/evidence pass is shipped and verified in both consumers. The surviving
local loops are compatibility or product-policy boundaries, not unfinished
bulk extraction. Further core work is defect-driven or duplication-driven
hardening behind the Hosted Telegram/web launch; it must not become a reason to
move Home Field policy, procedural art direction, or agent-workflow state into
core.
...
4. **Replace and approve the 12 Meat production assets.** The shared pipeline,
   ...
   `starter_port`/prototype status and Mushroom-derived ids/copy. Produce
...
- Preserve the dependency order core -> both consumers -> hub pointers, and
  require `npm run verify:backpack-core` on one shared SHA.
- Core owns neutral images, masks, rectangles, hashes, records, DTOs, and
  injected module factories. Products keep catalogs, thresholds, prompts,
  acceptance decisions, credentials, persistence, routes, and final layouts.
- A primitive is cross-game stable only after visible or authoritative use in
  both games. A utility proven by several independent Mushroom paths may remain
  provisional, but must not be presented as second-consumer validated.
...
- Immutable generated evidence is self-hashed and verifiable; mutable verdict,
  ...
  immutable evidence.
- Keep the core synchronous runner limited to bounded commands. Dev servers,
  Playwright lifecycle, provider streaming, deployment topology, and long-lived
  processes remain product-owned until two consumers need the same lifecycle.

The numbered list below is the historical implementation log that led to this
queue. It is retained for traceability and is not a second active TODO list:

1. **Completed 2026-07-13: move `gacha-admin-service.js` to core.** The service
   now lives in the quarantined Mushroom economy port and Mushroom's old import
...
3. **Completed 2026-07-13: move settlement/ops services.**
   `provider-settlement-service.js` and `wallet-ops-check-service.js` now live
   in the quarantined core economy port behind injected persistence, provider
...
5. **Bot gateway port completed 2026-07-13.** `bot-gateway.js` was physically
   moved into core as `createTelegramBotGatewayPort()`. Auth-code and payment
...
9. **Completed 2026-07-13: shared run lifecycle execution.** Core commit
   `2fa5c50` added persistence-neutral `run-runtime/v1`. Mushroom now executes
   it for real start/history/get/abandon/shop/solo/challenge-resolution routes
   and its long solo-run scenario. Meat executes the same factory for start,
   get, abandon, shop, loadout, battle, and replay over its atomic snapshot
   store, with HTTP abandonment coverage. SQL/snapshot transactions, catalogs,
   product rewards and policies, SSE, errors, and final route mounting remain
   product-local. The stable contract contains no Mushroom tables or words.
10. **Completed 2026-07-13: first visible cross-product page adoption.** The
   Mushroom `RunSummaryScreen` source was physically moved and neutralized in
   core commit `8f09c57`. It accepts one locale-ready summary DTO and emits
   neutral `home`/`open-round` events. Mushroom keeps a thin adapter at its old
   path for run state, Mushroom catalog/portrait lookup, locale text, and route
   events. Meat visibly mounts the same component after completed runs through
...
feature route groups, readiness module, `profile-runtime/v1`, and
`run-runtime/v1` execution slices are implemented. Both products also visibly
consume the same run-summary page shell. Future factories or page moves must be
driven by a concrete duplicated workflow in both products rather than
...
and economy ports through `gacha-admin-service.js` are now done; the next
physical move should start with support-money/support-ops, then
settlement/ops, before route-module ports.
...
- ✅ Done 2026-07-08 in core commit `de56ec8`: moved
  `app/server/services/game-service.js` into the quarantined
  `server/ports/mushroom/gameplay` package subpath as
  `createMushroomGameServicePort()`. Mushroom keeps the old service path as a
  thin wrapper and compatibility re-export facade that injects player state,
  run history, battle history, daily limit, home-field, asset pack,
  runtime-catalog, gacha-toggle, and direct-buy policy providers.
- ✅ Done 2026-07-08 in core commit `db3d77d`: moved
  `app/server/services/player-service.js` into the quarantined
  `server/ports/mushroom/gameplay` package subpath as
  `createMushroomPlayerServicePort()`. Mushroom keeps the old service path as
  a thin wrapper that injects DB, transaction, catalog, wallet, asset, season,
  clock, ID, bot-loadout, and lazy run-challenge providers.
- ✅ Done 2026-07-08 in core commit `fd78ad7`: moved
  `app/server/services/run-service.js` into the quarantined
  `server/ports/mushroom/gameplay` package subpath as
  `createMushroomRunServicePort()`. Mushroom keeps the old service path as a
  thin wrapper that injects DB, transaction, catalog, shop, battle, loadout,
  fusion, season, wallet, asset, clock, ID, RNG, and lock providers, plus the
  existing shop re-export facade.
- ✅ Done 2026-07-08 in core commit `c0c6111`: moved
  `app/server/services/wallet-service.js` into the quarantined
...
- ✅ Done 2026-07-13 in core commits `23ce46e` and `ede0499`: moved
  `app/server/services/gacha-admin-service.js` into the quarantined
  `server/ports/mushroom/economy` package subpath as
  `createMushroomGachaAdminServicePort()`. Mushroom keeps the old service path
  as a thin wrapper that injects DB/transactions, audit persistence, character
  and asset catalogs, wallet currency, clock/ID/JSON, env, and image storage.
- **Move next, quarantined after wallet/assets/gacha-admin:**
...
2. Move the smallest gameplay/profile server ports first:
   ...
   `6a9e3d8`; ✅ `shop-service.js` is done in core commit `bee8d43`; ✅
...
Status: **Active launch-hardening lane, actualized 2026-07-15.**
...
actor metadata, support mutations, asset provenance, and the shared production
asset review pipeline. Remaining launch work is real hosted deploy validation,
PostgreSQL migration/recovery evidence, deploy security/operator drills, the 12
approved Meat asset outputs, and explicit auto-pack/payment/gacha scope
decisions. Paid/gacha reconciliation is required only if those features are
enabled for the selected hosted target.
...
Status: **Pipeline implemented through 2026-07-14; content remains blocked.**
Meat starter artifacts, characters, and profile assets carry provenance and
adult-content policy metadata. The shared candidate/review pipeline now renders
real alpha-edge diagnostics, accepts isolated candidate roots, emits schema-v2
self-hashed evidence, and is covered by deterministic tests. The release gate
still correctly fails because all 12 distinct production replacements are
missing; placeholders are not acceptable for the selected public target.
...
- **Asset replacement content:** the production replacement pipeline, review
  sheet, alpha diagnostics, provenance evidence, and release gate now exist.
  The remaining blocker is the 12 Meat-owned approved outputs themselves.
...
- **Core extraction follow-up:** closed as a broad lane. Shared profile/run
  runtimes, route descriptors, frontend shells/components, config/auth/support
  DTOs, and script tooling are adopted by both products. Future movement must
  be a small response to concrete duplicated behavior and pass the same-SHA
  cross-consumer gate; do not inventory product files merely to find more code
  to move.
...
85. Phase 12 Meat Master product parity MVP. **Implemented 2026-07-05 for the
   ...
   Current Remaining Work Matrix. Remaining work is hosted production proof:
   PostgreSQL migrations/recovery, live Telegram deployment, operator security,
   12 approved Meat-owned assets, and explicit auto-pack/payment/gacha scope
   decisions.
...
87. Phase 13 post-implementation review findings. **Added 2026-07-05,
   actualized 2026-07-15:** before treating Meat as launch-ready, resolve
   migration/recovery evidence, support credential rotation, real Telegram
   deploy smoke, remaining production UI error coverage, the manual-editor
   decision, and the 12 production asset outputs. Paid/gacha ledger readiness
   is mandatory only if those features are enabled. The asset pipeline and
   broad core extraction are complete; further core movement is optional and
   must follow concrete duplication.
...
88. Phase 8AZ server route-module extraction. **Implemented 2026-07-13:** core
   ...
   now mounts profile/settings/active-character,
   ...
   rate-limit values, social authorization, and final Express app assembly
   remain local. Core also
...
90. Phase 8BB shared run lifecycle execution. **Implemented 2026-07-13 in
   ...
   - The operation matrix produced a small adapter-per-operation runtime rather
     than a shared persistence repository that would leak either schema.
   - Start, fetch/resume, shop mutations, battle resolution, replay, and
     completion/abandon execute through the stable factory in both products.
   - Existing HTTP response shapes and product transaction boundaries are
     preserved; stable arguments use neutral player/run/character/item names.
   - Daily limits, Mushroom challenge/Elo/season behavior, Meat tuning,
     rewards, SQL/snapshot stores, SSE, and HTTP mapping remain local.
   - Core contract tests and persistence-backed product tests prove execution.
     Challenge/readiness/community remain optional product capabilities rather
     than expanding the solo runtime interface.
91. Phase 8BC visible shared frontend shell adoption. **Implemented 2026-07-13
   in core commit `8f09c57` with real Mushroom and Meat rendering:**
   - `RunSummaryScreen` was selected because its original Mushroom source had
     no imports and a compact, testable state-to-view boundary.
   - The file was physically moved, then changed in place to accept a localized
     DTO and emit neutral actions without product imports or vocabulary.
   - Mushroom maps legacy run/catalog/locale state in a wrapper; Meat maps its
     run/character state and mounts the stable export as a Vue island.
   - Core component/export tests, Mushroom build and focused regression, and
     Meat build/full tests/browser completion prove real visible adoption.
