...
| `thalla_circle_canticle` | Thalla | Circle Canticle | Character mastery, late Thalla full-ring path. | Thalla drew one thread through the whole ring, and the spores answered in chorus. | Soft green medallion with a complete thread circle and three small echo-spores. |
...
| `lomie_ancient_bastion` | Lomie | Ancient Bastion | Character mastery, late Lomie low-loss path. | Lomie stood so calmly that the ring itself became a wall. | Moss-cream medallion with a rounded bark bastion and tiny root brace marks. |
...
| `axilin_cauldron_star` | Axilin | Cauldron Star | Character mastery, late Axilin high-win path. | Axilin's brew flared like a star, and the path hissed with victories. | Purple-grey medallion with a chunky cauldron starburst and two bubbling motes. |
...
| `kirt_unbroken_cadence` | Kirt | Unbroken Cadence | Character mastery, late Kirt steady-clear path. | Kirt crossed the ring without a missed beat, keeping measure strike by strike. | Warm rust-olive medallion with an unbroken rhythm loop and broad spear notch. |
...
| `morga_sunburst_crown` | Morga | Sunburst Crown | Character mastery, late Morga high-win path. | Morga lit the ring so brightly that victories settled around her like a crown. | Bright spark-cream medallion with a sunburst cap crown and one bold flash stroke. |
...
| `dalamar_null_crown` | Dalamar | Null Crown | Character mastery, late Dalamar control path. | Dalamar left a hush behind him where even armor forgot its shape. | Ash-teal medallion with a hollow crown shard and dark quiet center. |
