...
| Server module inventory and contract | `backpack-game-core/src/server/index.js`; `docs/server-module-contract.md`; Mushroom server files audited in S1 | Contract implemented, first helper move started | S1 classified DB/models, Telegram, payment callbacks, wiki/lore, social preview, support approvals, and final Express composition as product-local. The first low-risk helper move extracted artifact family capabilities; remaining server candidates include provider settlement adapters, ready-manager module-list wiring, gacha simulation service, and loadout-utils factory wiring. |
