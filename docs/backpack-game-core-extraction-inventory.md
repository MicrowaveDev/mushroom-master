...
- Highest-value remaining server move lane:
  `run-service.js`, `game-service.js`, and `player-service.js`. Move by
...
| `app/server/services/run-service.js` | Main gameplay spine port after the smaller repository patterns are proven. | Transactions, locks, challenge matching, persisted run/shop/loadout rows, battle resolver, rewards, rating, season awards, wallet grants, and public API compatibility aliases. |
