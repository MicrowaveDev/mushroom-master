...
| Server module inventory and contract | `backpack-game-core/src/server/index.js`; `docs/server-module-contract.md`; Mushroom server files audited in S1; `backpack-game-core/src/server/ports/mushroom/gameplay`; `backpack-game-core/src/server/ports/mushroom/economy`; `backpack-game-core/src/server/ports/mushroom/platform` | Route contract complete; real second-consumer execution is next | Core now has route descriptors and a quarantine strategy. Gameplay/profile services live under `src/server/ports/mushroom/gameplay`; wallet, asset, gacha-admin, support, provider-settlement, and wallet-ops services live under `src/server/ports/mushroom/economy`; auth/session/auth-code behavior, the adapterized Telegram bot gateway, and the filesystem/Markdown-configured wiki service live behind factories in `src/server/ports/mushroom/platform`. Auth, bot, wiki, profile, wallet, asset, social, run, support-admin, and gacha-admin route groups are complete. `social-preview-cache.js` is already a thin adapter over the stable core service. The next backend work is to neutralize one quarantined service contract and make Meat execute it in real API/browser flows instead of only smoke-testing exports. Product apps keep concrete DB/model/migration implementations, transactions, provider callbacks, credentials, middleware order, static file serving, deploy config, realtime/SSE delivery, and product catalogs. |
