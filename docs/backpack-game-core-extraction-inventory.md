...
| Server module inventory and contract | `backpack-game-core/src/server/index.js`; `docs/server-module-contract.md`; Mushroom server files audited in S1; `backpack-game-core/src/server/ports/mushroom/gameplay`; `backpack-game-core/src/server/ports/mushroom/economy` | Contract implemented, gameplay/economy quarantine expanded | Core now has route descriptors and a quarantine strategy. Gameplay/profile services live under `src/server/ports/mushroom/gameplay`; wallet, asset, gacha-admin, support, provider-settlement, and wallet-ops services live under `src/server/ports/mushroom/economy`; mutation claims live in the stable server facade. Remaining Mushroom server work is route/module extraction from auth/bot/wiki/social/create-app sections. Already core-backed wrapper files should be shrunk rather than bulk-moved again. Product apps keep concrete DB/model/migration implementations, transactions, provider callbacks, credentials, middleware order, static file serving, deploy config, realtime/SSE delivery, and product catalogs. |
