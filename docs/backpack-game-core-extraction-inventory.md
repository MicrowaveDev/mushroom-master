...
| Server module inventory and contract | `backpack-game-core/src/server/index.js`; `docs/server-module-contract.md`; Mushroom server files audited in S1 | Contract implemented, module moves pending | S1 classified DB/models, Telegram, payment callbacks, wiki/lore, social preview, support approvals, and final Express composition as product-local. The best first server module candidates are pure/adapted helpers such as provider settlement adapters, artifact helpers, ready-manager module-list wiring, gacha simulation service, and loadout-utils factory wiring. |
