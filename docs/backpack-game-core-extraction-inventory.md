...
- quarantined gameplay ports: `src/server/ports/mushroom/gameplay` exposes
  ...
  `createMushroomGameServicePort()`, and `createMushroomPlayerServicePort()`
  after physically moving Mushroom's matching service files into core. The
  ports preserve current Mushroom SQL/table and profile/bootstrap DTO behavior
  behind injected providers. Mushroom keeps the old service paths as wrappers;
  Meat verifies the exports without adopting these temporary Mushroom table
  contracts.
...
- Highest-value remaining server move lane:
  `run-service.js`. Move by `mv` into a quarantined core port first, then
  neutralize DB/catalog/policy access behind repositories and adapters.
