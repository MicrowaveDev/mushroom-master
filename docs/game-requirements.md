...
- **15-X.** Home Field chibi approval is blocked unless the sprite satisfies the canon-facing chibi requirements in `docs/design-requirements.md` Section 11 and the Stage 1 chibi contract. A mechanically valid spritesheet is not enough: candidates that read as pixel art, a tiny beige doll, a generic elf, a straight portrait sticker, or a human with a mushroom hat must be marked `rejected`/`needs_review`, excluded from active-scene production validation, and regenerated as candidate-only evidence before roster expansion.
