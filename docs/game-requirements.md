...
- **13-E.** Replay combat readability is primary. Portraits, speech bubbles, health, damage/stun feedback, and the combat log must remain visually dominant over supporting loadout context. Speech bubbles must read as coming from the speaking character without covering the character's face/head: each portrait variant used in replay must have configured framing, bubble placement, and a head/face band. Replay screenshot tests must assert the bubble body stays inside the portrait image, outside that band, and above the name overlay, preferably near the head with the tail pointing back toward the speaking character.
