...
- Setting the sheet to `overflow: visible` — collides with the sheet's rounded-corner clipping rules (see the `replay-sheet-toggle` / `replay-sheet-body` `border-radius` pairing that exists *because* the parent doesn't clip — the mushroom decoration relies on this).
