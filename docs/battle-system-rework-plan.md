...
- the run-complete screen also shows a persisted cosmetic season rank from cumulative season points. Each finished run adds `min(wins, 7) * 2 - losses`, plus `+3` for a max-round clear. Abandon applies `-2` before any completed round or `-5` after progress. Persisted totals clamp at 0, and the player-season row stores peak points/rank separately for season-end rewards. `npm run game:season:archive -- --write` archives a season and grants rewards from peak rank once. It powers recap presentation and lore achievement criteria only; it does not affect combat, matchmaking, shop offers, ghosts, rewards, or rating. The current named chapter is `Season of the Deep Ring`; progress is not reset destructively.
...
- prep shows the current season-rank stakes before Ready/Abandon. Home shows the current and peak season rank/progress, recent achievement unlocks, and a next-badge hint when no recent unlocks exist; profile contains the achievement journal grouped by Season, General, and Character. Run-complete animates verdict, progress, rank-up, rank-down penalty state, achievements, empty-achievement state, and final action for game-feel. Sound/haptic hooks are emitted as browser events, and client analytics posts achievement/rank events to `/api/client-events`; the backend persists those rows so `npm run game:season:analytics` can summarize rank movement, abandons, demotions, opponent kind, and repeated-opponent patterns. Sound sourcing guidance is in `docs/sound-design-recommendations.md`.
- focused season-rank tuning and operator notes live in [`season-ranking.md`](season-ranking.md)
