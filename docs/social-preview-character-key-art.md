...
The current committed base image at `web/public/marketing/character-key-art-base.png` was generated from Prompt A: Premium Hero Splash. The production social preview at `web/public/marketing/social-preview.jpg` uses the `middle-bottom` layout with the `storybook` style. It is generated by compositing the stylized title with:

```bash
npm run game:social-preview -- --production
```

On app startup, the server refreshes the served cache at `web/dist/marketing/social-preview.jpg` from the same base image and title template. If image generation is unavailable in the runtime, the committed `web/public/marketing/social-preview.jpg` is used as the fallback. The production preview is JPEG because social cards are full-bleed illustration art with no transparency, and JPEG keeps the file much smaller than PNG.
