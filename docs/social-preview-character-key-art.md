...
The current committed base image at `web/public/marketing/character-key-art-base.png` was generated from Prompt A: Premium Hero Splash. The production social preview at `web/public/marketing/social-preview.png` uses the `middle-bottom` layout with the `storybook` style. It is generated by compositing the stylized title with:

```bash
npm run game:social-preview -- --production
```

On app startup, the server refreshes the served cache at `web/dist/marketing/social-preview.png` from the same base image and title template. If image generation is unavailable in the runtime, the committed `web/public/marketing/social-preview.png` is used as the fallback.
