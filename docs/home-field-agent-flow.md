...
For chibi runs, the Prompt/Contract Reviewer must state a palette plan before the Imagegen Worker starts: `12-18` artist-visible colors, fewer than `20` total design colors, transparency and `#ff00ff` excluded, and shared cap/robe/skin/gold ramps instead of many near-duplicate local shades. The same note must state the style-preservation plan: preserve the compact grouped-sheet charm from `docs/reference/home-field/chibi-thalla-previous-best-2026-06-26-state-sheet.png` while fixing palette bloat and ornament. This is a run note/checkpoint, not a new PNG artifact.
