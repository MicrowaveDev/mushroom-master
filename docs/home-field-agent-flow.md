...
HOME_FIELD_CANDIDATE_ROOT=.agent/home-field-workspace/candidates/terrain-family/latest HOME_FIELD_CANDIDATE_IDS=<family_ids> npm run game:home-field:preview -- --scope=terrain
```
...
Before path promotion specifically, inspect the path candidate together with the current grass candidate/baseline in `game:home-field:preview -- --scope=combined`. A path family can be internally coherent and still fail production if the path tiles look pasted onto the grass field.
...
npm run game:home-field:produce -- --scope=objects --candidate -- ${OBJECT_IDS//,/ } --resize --chroma-key=#ff00ff
...
HOME_FIELD_CANDIDATE_ROOT=.agent/home-field-workspace/candidates/object-layer/latest HOME_FIELD_CANDIDATE_IDS=$OBJECT_IDS npm run game:home-field:preview -- --scope=objects
```
...
Palette checkpoint: before generating the reference sheet, the Imagegen Worker should be operating from the explicit palette plan above. Stop at the reference gate if the turnaround already looks like a soft illustration palette, uses many cream/beige/blush/gold micro-tones, or appears to exceed roughly `20` visible design colors. Also stop if the palette rule has been overcorrected into hard pixel art, clean vector/cel icon art, a large flat anime/fashion/showcase turnaround, a cute-but-generic anime/chibi character sheet, quadrant-filling character art, glossy anime eyes, large white eye shines, eyelashes, hair/wig under a mushroom cap, earrings/jewelry styling, royal regalia, crown jewels, forehead gems, brooches, chest medallions, pendants, clasps, jewelry-like cap crests, scalloped collars, ornamental robe borders, decorative trim clusters, sleeve cuff trim, collar jewels, repeated gold badges, a tiny uniform mascot/token read, or a cold "exactly 16 swatches" exercise that loses the previous-best compact charm. Review the final grouped state sheet and composed candidate the same way. Use `game:home-field:palette-audit` to generate JSON and swatch evidence, but keep the decision visual: raw unique-RGBA count after antialiasing or resize is diagnostic only. If palette bloat, palette overcorrection, generic anime styling, cap-hair drift, glossy-eye drift, status-ornament drift, relative-improvement-only style recovery, or high-res polished turnaround normalization drift is visible, the Visual Critic must fail `styleCohesionCheck` and `stageContractCheck`, set `verdict: "needs_regen"`, and name the palette/style failure in `reason`.
...
Run `game:home-field:preview -- --scope=grass` before visual review. It uses Playwright route interception to render `/home-field-preview?debug=0` with the candidate grass PNGs from `.agent/home-field-workspace/candidates/grass-family/latest/`, without promoting or overwriting app-facing PNGs. The app-facing clean preview screenshots under `.agent/tasks/...` remain post-promotion proof.

Run `game:home-field:preview -- --scope=objects` for the foliage micro-batch. It uses the same clean preview screenshot paths, but routes `bush_cluster_dark_01`, `bush_cluster_light_01`, and `leaf_sprout_01` from `.agent/home-field-workspace/candidates/object-layer/latest/`.
