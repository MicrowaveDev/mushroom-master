...
- treat the queue-backed built-in same-context staging path as the current allowed method change for one fresh reference-attempt batch only; stop if that path is unavailable or fails the same visual gate twice;
