...
- Hardened chibi preflight so chat-visible built-in imagegen output is not enough. The proof now requires confirmed discoverable PNG output, supplied local proof source images outside `docs/reference`, or explicit CLI fallback credentials before cleanup/generation starts.
