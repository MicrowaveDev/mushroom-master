...
- `footprintType`: derived from the displayed artifact footprint. Normal artifacts use `artifact.width` / `artifact.height`; bags use `getBagShape(artifact)` so legacy rectangular bags such as `moss_pouch` classify by the landscape 2×1 display shape rather than their persisted 1×2 data. Values: `single` (1×1), `wide` (`cols > rows`, no shape mask), `tall` (`rows > cols`, no shape mask), `block` (`cols === rows` and >1, no shape mask), `mask` (bag with an explicit `shape` array, e.g. T/L/J/S/Z/I tetrominoes). **Bag-only for `mask`.** **Tracks the canonical display orientation**, not the player's current rotation — rotation is presentation, not identity, so consumers (Phase 5 replay highlights) can use the canonical type and apply orientation styling separately.
