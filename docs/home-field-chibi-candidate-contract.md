...
- `docs/reference/home-field/chibi-thalla-liked-2026-06-23.png`: primary positive youthful little-girl face/cap/robe appeal, cute dark storybook warmth, simple BJD-inspired chibi doll face, smooth BJD-like face planes, compact visible oval/almond doll eyes, rounded cheeks, and soft cute expression; simplify heavily.
...
If palette cleanup, quantization, chroma repair, resize repair, or external palette-aware tools are used, compare the raw generated sheet, repaired source, final candidate, and the two positive Thalla references before recording a verdict. Repair fails if it flattens the old-reference charm into a hard toy, flat icon, generic mascot, or tiny uniform token, or if it erases the soft doll-face warmth, rounded cheeks, compact visible oval/almond eyes, or cute little-girl expression even when palette audits pass. Do not accept a repair because it is merely better than older rejected lineages; the candidate must match the positive references on an absolute basis.
...
- no detached motion/action lines, squiggle marks, speed lines, punctuation-like accents, text marks, stray specks, baked floor/background bits, or other non-character components outside the main sprite body; chibi-quality validation rejects detached mark clusters
...
- no cute-but-generic anime/chibi character sheet; large glossy/white eye shines, visible hair/bangs/wig fringe under the mushroom cap, or brooch/medallion/clasp-like chest ornament are `needs_regen` even if the sheet is cute and mechanically clean
...
Mechanical checks are necessary but not sufficient. A candidate that passes dimensions, alpha, sheet mapping, or mobile readability still fails if it reads as pixel art, a tiny beige featureless doll, a busy ornate fantasy sprite, a generic elf, a straight portrait sticker, a human with a mushroom hat or hair under a mushroom cap, glossy anime-eyed turnaround art, cute generic anime/chibi character-sheet art, quadrant-filling character art, an old mushroom monk, an elderly gnome, a beige mascot pawn, a faceless mushroom token, a tiny uniform mascot/token, a skull-mask face, hollow pin-dot eyes, a blank mask face, or a different renderer from the Home Field reference. Do not upgrade `needs_regen` or failed visual checks to `needs_review` from validator passes, palette audits, or self-authored comparison prose; clear them only after direct positive-reference visual review.
