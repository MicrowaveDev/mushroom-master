...
If palette cleanup, quantization, chroma repair, resize repair, or external palette-aware tools are used, compare the raw generated sheet, repaired source, final candidate, and the two positive Thalla references before recording a verdict. Repair fails if it flattens the old-reference charm into a hard toy, flat icon, or generic mascot, or if it erases the soft doll-face warmth, rounded cheeks, compact visible oval/almond eyes, or cute little-girl expression even when palette audits pass.
